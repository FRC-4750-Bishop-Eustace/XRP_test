...
class DifferentialDrive:
    ...
    def __init__(self, left_motor: EncodedMotor, right_motor: EncodedMotor, motor_three: EncodedMotor, motor_four: EncodedMotor, imu: IMU = None, wheel_diam:float = 6.0, wheel_track:float = 14.5):
        """
        ...
        self.motor_three = motor_three
        self.motor_four = motor_four
        self.imu = imu

        self.wheel_diam = wheel_diam
        self.track_width = wheel_track

    def set_effort(self, left_effort: float, right_effort: float, three_effort: float, four_effort: float) -> None:
        """
        ...
        self.motor_three.set_effort(three_effort)
        self.motor_four.set_effort(four_effort)

    def set_speed(self, left_speed: float, right_speed: float, three_speed: float, four_speed: float) -> None:
        """
        ...
        self.motor_three.set_speed(three_speed*cmpsToRPM)
        self.motor_four.set_speed(four_speed*cmpsToRPM)
    ...
    def stop(self) -> None:
        ...
        self.motor_three.set_speed()
        self.motor_four.set_speed()
        self.set_effort(0,0,0,0)

    def arcade(self, straight:float, turn:float, strafe:float):
        """
        ...
        :type turn: float
        """
        if straight == 0 and turn == 0 and strafe_left == False and strafe_right == False:
            self.set_effort(0, 0, 0, 0) 
        else:
            scale = max(abs(straight), abs(turn))/(abs(straight) + abs(turn))
            left_speed = (straight - turn)*scale
            three_speed = (straight - turn)*scale
            right_speed = (straight + turn)*scale
            four_speed = (straight + turn)*scale
            self.set_effort(left_speed, right_speed, three_speed, four_speed)
    ...
    def reset_encoder_position(self) -> None:
        ...
        self.motor_three.reset_encoder_position()
        self.motor_four.reset_encoder_position()
    ...
    def get_three_encoder_position(self) -> float:
        """
        :return: the current position of the right motor's encoder in cm.
        :rtype: float
        """
        return self.motor_three.get_position()*math.pi*self.wheel_diam

    def get_four_encoder_position(self) -> float:
        """
        :return: the current position of the right motor's encoder in cm.
        :rtype: float
        """
        return self.motor_four.get_position()*math.pi*self.wheel_diam
    ...
    def straight(self, distance: float, max_effort: float = 0.5, timeout: float = None, main_controller: Controller = None, secondary_controller: Controller = None) -> bool:
        ...
        starting_three = self.get_three_encoder_position()
        starting_four = self.get_four_encoder_position()
